...
public class BoardDao {
	...
	public int boardInsert(BoardVo bv) {
		...
		String sql = "INSERT INTO BOARD_AWS(originBidx, depth, level_, subject, contents, writer, password, filename, ip)\r\n"
				   + "value(null, 0, 0, ?, ?, ?, ?, ?, ?)";
		String sql2 = "update BOARD_AWS \r\n"
	            + "set originBidx = (select * from (select max(bidx) from BOARD_AWS) as temp) \r\n"
	            + "where bidx = (select * from (select max(bidx) from BOARD_AWS) as temp)";
		String sql3 = "SELECT MAX(bidx) AS maxbidx FROM BOARD_AWS";
		try {
